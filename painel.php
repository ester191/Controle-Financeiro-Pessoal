...
if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== TRUE) {
    ...
    $url_alerta = $caminho_base . 'index.php?alerta=acesso_restrito';
    
    echo '
    <!DOCTYPE html>
    <html>
    <head>
        <meta http-equiv="refresh" content="0; url=' . $url_alerta . '">
        <title>Acesso Negado</title>
    </head>
    <body>
        <h1>Acesso Negado</h1>
        <p>Você precisa fazer login. Redirecionando...</p>
        <a href="' . $url_alerta . '">Clique aqui se o redirecionamento falhar.</a>
    </body>
    </html>';
...
$receitas_totais = 0.00;
$despesas_totais = 0.00;
$saldo_total = 0.00;
$transacoes_recentes = [];

try {
    $sql_receitas = "SELECT SUM(valor) AS total_receitas FROM transacoes WHERE user_id = :user_id AND tipo = 'receita'";
    $stmt_receitas = $pdo->prepare($sql_receitas);
    $stmt_receitas->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $stmt_receitas->execute();
    $receitas_totais = $stmt_receitas->fetchColumn() ?: 0.00;

    $sql_despesas = "SELECT SUM(valor) AS total_despesas FROM transacoes WHERE user_id = :user_id AND tipo = 'despesa'";
    $stmt_despesas = $pdo->prepare($sql_despesas);
    $stmt_despesas->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $stmt_despesas->execute();
    $despesas_totais = $stmt_despesas->fetchColumn() ?: 0.00;

    $saldo_total = $receitas_totais - $despesas_totais;


    $sql_recentes = "SELECT tipo, descricao, valor, data_transacao, categoria FROM transacoes 
                     WHERE user_id = :user_id 
                     ORDER BY data_transacao DESC, data_registro DESC 
                     LIMIT 5"; 
    $stmt_recentes = $pdo->prepare($sql_recentes);
    $stmt_recentes->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $stmt_recentes->execute();
    $transacoes_recentes = $stmt_recentes->fetchAll(PDO::FETCH_ASSOC);


    $mes_atual = date('Y-m');
    $sql_grafico = "SELECT categoria, SUM(valor) AS total_gasto 
                    FROM transacoes 
                    WHERE user_id = :user_id 
                    AND tipo = 'despesa' 
                    AND data_transacao LIKE :mes_atual_like 
                    GROUP BY categoria 
                    ORDER BY total_gasto DESC";
    
    $stmt_grafico = $pdo->prepare($sql_grafico);
    $stmt_grafico->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $stmt_grafico->bindValue(':mes_atual_like', $mes_atual . '-%', PDO::PARAM_STR);
    ...
    foreach ($dados_grafico as $dado) {
        $categorias_grafico[] = $dado['categoria'];
        $valores_grafico[] = floatval($dado['total_gasto']);
    }
    
    $json_categorias = json_encode($categorias_grafico);
    $json_valores = json_encode($valores_grafico);
...
function get_category_icon($categoria, $tipo) {
    if ($tipo == 'receita') {
        switch ($categoria) {
            case 'salario': return 'fas fa-briefcase';
            case 'freelance': return 'fas fa-laptop-code';
            case 'investimento': return 'fas fa-chart-line';
            default: return 'fas fa-gift';
        }
    } else { 
        switch ($categoria) {
            case 'aluguel': return 'fas fa-home';
            case 'alimentacao': return 'fas fa-shopping-cart';
            case 'transporte': return 'fas fa-car';
            case 'lazer': return 'fas fa-film';
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo $caminho_base; ?>css/principal.css"> 
...
<body>
    ...
    <header class="header">
        ...
        <nav class="navbar">
            <ul>
                <li><a href="<?php echo $caminho_base; ?>painel.php" class="active"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
        ...
        <div class="profile-dropdown-container">
            <div class="user-profile">
                ...
                <img src="<?php echo $caminho_base; ?>assets/imagemusuario.webp" alt="Foto de Perfil">
            </div>

            <div class="dropdown-menu" id="userDropdown">
                 <a href="<?php echo $caminho_base; ?>logout.php"><i class="fas fa-sign-out-alt"></i> Sair</a>
            </div>
        </div>
    </header>
            ...
            <div class="right-column">
                <section class="quick-actions card">
                    ...
                    <div class="actions-grid">
                        ...
                        <a href="<?php echo $caminho_base; ?>receitas.php" class="action-item action-recipe">
                            <i class="fas fa-plus-circle"></i>
                            <p>Receita</p>
                        </a>
                        <a href="<?php echo $caminho_base; ?>despesas.php" class="action-item action-expense">
